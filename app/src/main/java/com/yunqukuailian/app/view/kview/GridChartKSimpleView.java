...
public class GridChartKSimpleView extends View {
     ...
     * 默认字体大小
     **/
    public float DEFAULT_AXIS_TITLE_SIZE = 12;
    public float DEFAULT_AXIS_TITLE_HINTSIZE = 10;
    public boolean isShowTitle = false;

    public static int DEFAULT_RED = 0xE01324;
    public static int DEFAULT_GREEN = 0x25A749;
     ...
     * 默认上表纬线数
     */
    public int DEFAULT_UPER_LATITUDE_NUM = 6;
     ...
     * 默认中表纬线数
     */
    public static int DEFAULT_MIDDLE_LATITUDE_NUM = 4;
    /**
     * 默认下表纬线数
     */
    public static int DEFAULT_LOWER_LATITUDE_NUM = 4;
    ...
    public void setDefauliMiddleLatitudeNum(int postion){
        this.DEFAULT_MIDDLE_LATITUDE_NUM= postion;
    }
    ...
    public GridChartKSimpleView(Context context) {
        super(context);
        init(context);
    }

    public GridChartKSimpleView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context);
    }

    public GridChartKSimpleView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mMiddleChartHeight = 1 * latitudeSpacing;
...
//        canvas.drawLine(0,viewHeight-mLowerChartHeight-Up_chart_margin/2,viewWidth,viewHeight-mLowerChartHeight-Up_chart_margin/2,paint);
//        canvas.drawLine(0,viewHeight-mLowerChartHeight-mMiddleChartHeight-Up_chart_margin*3/2,viewWidth,viewHeight-mLowerChartHeight-mMiddleChartHeight-Up_chart_margin*3/2,paint);
    }
        ...
        for (int i = 0; i < DEFAULT_UPER_LATITUDE_NUM; i++) {
            ...
            if (axisYTitles != null) {
                ...
                if (i == DEFAULT_UPER_LATITUDE_NUM -1) {
                    ...
                    canvas.drawText(axisYTitles.get(DEFAULT_UPER_LATITUDE_NUM+1), x, TITLE_HEIGHT+DisplayUtil.dip2px(getContext(),10), paintAxis);
        ...
        try {
            ...
            for (int i = 0; i < DEFAULT_LOGITUDE_NUM - 1 && i < axisXTitles.size(); i++) {
                ...
                if (Position.Top == mPosition) {
                    if (isInnerYAxis) {
                        canvas.drawText(axisXTitles.get(i), super.getWidth() - longitudeSpacing * (i + 1) - tWidth, 2*mUperChartHeight - DisplayUtil.dip2px(getContext(),10) , paintAxis);
                    } else {
                        canvas.drawText(axisXTitles.get(i), super.getWidth() - DEFAULT_AXIS_MARGIN_RIGHT - longitudeSpacing * (i + 1) - tWidth, 2*mUperChartHeight - DisplayUtil.dip2px(getContext(),10), paintAxis);
                    }

                } else {
                    if (isInnerYAxis) {
                        canvas.drawText(axisXTitles.get(i), super.getWidth() - longitudeSpacing * (i + 1) - tWidth, getHeight() , paintAxis);
                    } else {
                        canvas.drawText(axisXTitles.get(i), super.getWidth() - DEFAULT_AXIS_MARGIN_RIGHT - longitudeSpacing * (i + 1) - tWidth, getHeight() , paintAxis);
                    }
    ...
    public float adjustFontSize(int screenWidth, int screenHeight) {
        ...
        float rate = ((float) screenWidth / 160);
